        ...
        if (! parallel) {
            ...
            for (; iters < max_iters; iters++) {
                ...
                for (int i = 0; i < number_of_models; i++) {
                    ...
                    if (current_score.isBetter(best_score)) {  // 如果最近的model更好
...
bool run (const Ptr<const Model> &params, InputArray points1, InputArray points2, int state,
    ...
    if (params->isPnP()) {
        if (! K1_.empty()) {
            ...
            if (! dist_coeff1.empty()) {
                ...
                points_size = mergePoints(undist_points1, points2, points, true);  // 水平拼接两个points
    ...
    Ransac ransac (params, points_size, estimator, quality, sampler,
          termination, verifier, degeneracy, lo, polisher, params->isParallel(), state);  // estimator和degeneracy包含了points
